...
{{--        <h2 class="flex-grow text-center pb-2 text-white text-2xl ">Plaats hier uw bestelling</h2>--}}
    ...
    <div class="sm:ml-7 sm:mr-7 sm:mb-10">
        <form wire:submit.prevent="submitForm" class="">
            ...
            @endif
            <table class="table-auto md:mx-auto bg-[#f5f7fa] shadow-2xl">
                <thead class="text-left bg-[#dcdee1]">
                <tr class="text-lg [&>th]:text-[#192c44] [&>th]:p-4">
                    <th>Naam</th>
                ...
                @forelse($products as $index => $product)
                    <tr class="[&>td]:p-2">
                        <td class="border-y border-[#d4dde9]">
                            <div class="text-[#617691] p-2">
                                {{ $product->name }}
                            </div>
                            <div class="sm:hidden">
                                <select
                                    wire:model="selectedSize.{{ $index }}"
                                    class="ml-2 text-[#11253e] rounded-md text-sm border-[#d4dde9]">
                                    <option value="" class="text-[#11253e]">Kies je maat</option>
                                    @foreach($this->getSizesForSelectedProduct($product->id, $index) as $size)
                                        <option value="{{ $size->id }}" class="text-[#11253e] text-md">{{ $size->size }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="sm:hidden py-4 ">
                                <input type="number" class="w-20 ml-2 text-[#11253e] rounded-md text-sm" min="0" max="10"
                        ...
                        <td class="border-collapse border-y border-[#d4dde9] text-[#617691]">
                            € {{ $product->price }}</td>
                        ...
                        <td class="border-y border-[#d4dde9] text-[#617691]">
                ...
                <tfoot>
                <tr class="[&>td]:text-[#617691] [&>td]:p-2 [&>td]:text-lg [&>td]:font-semibold">
                    <td></td>
