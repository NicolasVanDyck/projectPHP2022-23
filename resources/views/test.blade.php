...
<div style="border: thick black;" ></div>

<hr>
{{--NEW PARAMETER--}}
<h1 style="font-weight: bold">new Parameter</h1>
<form method="POST" action="{{ route('parameter.store') }}">
    @csrf
    <label for="end_date_order">End Date Order:</label>
    <input type="date" timezone="" name="end_date_order">
    <button type="submit">Create Parameter</button>
</form>

<hr>
{{--DELETE PARAMETER--}}
<h1 style="font-weight: bold">Delete Parameter</h1>
<form method="POST" action="{{ route('parameter.destroy') }}">
    @csrf
    @method('DELETE')
...
<form method="POST" action="{{ route('parameter.update') }}">
    ...
    <select id="current_end_date_order" name="parameter_id">
        @foreach ($parameters as $parameter)
            <option value="{{ $parameter->id }}">{{ $parameter->end_date_order }}</option>
        @endforeach
    </select><br><br>
    <label for="new_end_date_order">New End Date Order:</label>
    <input type="date" id="new_end_date_order" name="end_date_order"><br><br>
